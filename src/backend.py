...
class User:
    ...
    def existe_utilisateur(self, utilisateur_id):
        ...
        self.cursor.execute("SELECT EXISTS(SELECT 1 FROM profil WHERE utilisateurId=%s)", (utilisateur_id,))
    ...
    def inserer_score(self, utilisateur_id, meilleur_score):
    ...
    def afficher_score(self, utilisateur_id):
        """Renvoie le meilleur score d'un utilisateur donné."""
        self.cursor.execute("SELECT meilleurScore FROM profil WHERE utilisateurId=%s", (utilisateur_id,))
        meilleur_score = self.cursor.fetchone()  # Récupérer le meilleur score
